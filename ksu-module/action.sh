#!/system/bin/sh
# KernelSU Manager Action opens the helper at the user's explicit tap.
...
case "$_act_mode" in
    open)
        ...
        ;;
    status)
        echo "Poco F7 CTAP2 KernelSU development module"
        echo "State: $STATE"
        echo "Boot: $(m_boot_id)"
        ...
        echo "Server PID (module-owned): $(daemon_owned_pid || echo none)"
        ...
        echo "Foreground biometric: open the Poco F7 CTAP 3B DEV app manually."
        echo "Recent logs:"
    ...
    start)
        ...
        m_init || exit 1
        rm -f "$STATE/stopping"
        daemon_stage_payload && daemon_launch ||
            { echo "CTAP daemon unavailable: check logs / open helper Activity"; exit 1; }
        echo "Android helper CTAP daemon started. Biometric UI must be foreground."
        ;;
    stop)
        m_init || exit 1
